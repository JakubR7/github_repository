...
func TestCreateRepoConcurrentNoError(t *testing.T) {
	...
	assert.EqualValues(t, "JakubR7", result.Response.Owner)
...
func TestCreateReposOneSuccessOneFail(t *testing.T) {
	...
	for _, v := range result.Results {
		if v.Error != nil {
			assert.Nil(t, v.Response)
			assert.EqualValues(t, http.StatusBadRequest, v.Error.Status())
			assert.EqualValues(t, "invalid repository name", v.Error.Message())
		} else {
			assert.Nil(t, v.Error)
			assert.EqualValues(t, 123, v.Response.Id)
			assert.EqualValues(t, "testing", v.Response.Name)
			assert.EqualValues(t, "JakubR7", v.Response.Owner)
		}
	}
...
func TestCreateReposAllSuccess(t *testing.T) {
	...
	assert.EqualValues(t, "JakubR7", result.Results[0].Response.Owner)
}
